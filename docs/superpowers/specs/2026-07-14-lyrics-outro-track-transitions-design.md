...
`TrackTransitionDirectionController`가 앞으로 발생할 `trackChanged`에 사용할 명시적 intent FIFO를 관리한다.

- PiP 다음 버튼: `{ direction: "next", createdAtMs }`를 enqueue한 뒤 `player.next()` 호출.
- PiP 이전 버튼: `{ direction: "previous", createdAtMs }`를 enqueue한 뒤 `player.previous()` 호출.
- `createdAtMs`는 버튼 callback에 들어온 시점의 monotonic clock 값이다. 컨트롤러는 테스트에서 clock을 주입받는다.
- `NAVIGATION_INTENT_TIMEOUT_MS`는 `5000`이다. `nowMs - createdAtMs > 5000`인 항목은 enqueue와 consume 전에 FIFO 앞에서부터 폐기한다.
- 실제 `trackChanged` 하나는 가장 오래된 유효 intent 하나만 dequeue해 소비한다. 두 번의 빠른 next 요청과 두 번의 trackChanged는 차례대로 두 개의 `next`를 소비한다.
- `next()`와 `previous()`가 결과를 반환하지 않으므로 실패를 별도로 관측하지 않는다. 이벤트가 오지 않은 요청은 5초 만료로만 정리한다.
- no-track, PiP close, destroy에서는 FIFO 전체를 비운다. 새 곡 로드·전환 완료만으로 다른 pending intent를 지우지 않는다.
...
- `previousProgressSec >= previousDurationSec - NATURAL_END_TOLERANCE_SEC`이면 자연 종료에 의한 `next`로 추론한다.
- 그렇지 않으면 `unknown`이다.
...
- incoming cover가 로드되면 outgoing cover와 `COVER_CROSSFADE_DURATION_MS = 360` 동안 교차 페이드한다.
...
1. 만료된 intent를 폐기하고 FIFO의 가장 오래된 명시적 intent 하나를 소비.
...
- 연속 next/previous: trackChanged마다 FIFO intent를 하나씩 소비하고 현재 transition은 최신 곡 scene으로 교체. 아직 대응하는 trackChanged가 오지 않은 나머지 intent는 유지.
...
- 명시적 next/previous intent의 FIFO 소비, 5초 만료, no-track/PiP close/destroy 초기화.
...
- rapid skip에서 trackChanged마다 FIFO 방향을 순서대로 소비하되 DOM에는 최신 track metadata와 최신 전환만 남음.
